...
lamb2_chose = 2;
iClus = 1;
[rowClus1, colClus1, z1, U, V, obj1] = mvlrrl1(M, lz, lamb1_chose * ones(length(M), 1), ...
...
[rowClus2, colClus2, z2, U, V, obj2] = mvlrrl1(M_r, lz, lamb1_chose * ones(length(M_r{1}), 1), ...
...
[rowClus3, colClus3, z3, U, V, obj3] = mvlrrl1(M_r, lz, lamb1_chose * ones(length(M_r{1}), 1), ...
    dimRatio * lamb2_chose, size(M_r{1}, 1), 'weighted');
fprintf('none zero in u1: %d, none zero in u2: %d, shared none zero: %d\n', ...
    length(find(U(:, 1) ~= 0)), length(find(U(:, 2) ~= 0)), ...
    length(find(U(:, 1) ~= 0 & U(:, 2) ~= 0)));
